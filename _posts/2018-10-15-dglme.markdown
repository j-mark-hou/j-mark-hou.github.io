...
- some relevant strands of work:
	...
	- there's also been some recent-ish work in math / stats on characterizing the limit behavior of various properties of dense graphs
		...
		- markets are basically just bipartite graphs with the two types of nodes being consumers and products
...
- consumers $$i = 1,...,I$$
	...
	u_i(x_i) = \prod_jx_{ij}^{\alpha_{ij}}, \quad\alpha_{ij}=1
	\label{cbutil}
	\tag{1}
	\end{equation}$$
...
- a graph's adjacency matrix is just a symmetric function from $$[0,1]\times[0,1]\mapsto \{0,1\}$$ 
	- disccretize the interval $$[0,1]$$ into $$|V|$$ intervals corresponding to the vertices of the graph
...
Some further work (Borgs et. al. 2007) put a norm on this graphon space (in fact, a more general space, as they allow for edge and node weights)
...
- the theorem states that these are equivalent (plus some other things we dont' care about):
	- (a) $$f$$ is testable
	- (d) $$f$$ can be extended to some $$\tilde{f}$$ defined on graphon space continuous in $$\delta_{\square}$$
	-  $$f$$ can be extended to some function $$\tilde{f}$$ on the corresponding graphon space that is continuous in the $$\delta_{\square}$$
	- (e.1-e.3):
		- $$\forall \epsilon, \enspace \exists \epsilon'$$ such that for any two graphs $$G, G'$$ on the same node set with $$d_{\square}(G,G')<\epsilon'$$, we have $$\lvert f(G)-f(G')\rvert<\epsilon$$
		- $$f(G[m])$$ converges as $$m\rightarrow\infty$$, where $$G[m]$$ represents the graph obtained by duplicating each vertex of $$G$$ $$m$$ times and adding in all the corresponding edges
		- $$f(G)-f(G')\rightarrow 0$$ as the number of vertices in $$G$$ goes to $$\infty$$, where $$G'$$ is just $$G$$ except with an additional disconnected node
...
- budget of a consumer node is its degree
...
- why normalize by n2 instead of fraction of total budget:
	- if first all consumers have degree1, each attached to a single type0 nodes, and then everyone gets another attachment to type1 nodes,
