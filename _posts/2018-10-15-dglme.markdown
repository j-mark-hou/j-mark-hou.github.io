...
#### III.2. Other related parameters that are not testable
We made some further assumptions to the simple example in order to apply this parameter testing stuff above.  Two of them are maybe a bit unnatural, and they were made because somehow more natural assumptions would have resulted in graph parameters that were not testable:
...
2. assigning each consumer $$i$$ a budget proportional to the number of items they like
	...
	- the key here is that a contingent of consumers with very few edges control a large share of the total revenue makes, so that small changes to the edge count can significantly change the revenue distribution.
