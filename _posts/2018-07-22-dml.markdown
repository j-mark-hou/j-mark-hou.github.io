...
1. use a score $$\phi$$ that satisfies a **Neyman orthogonality** condition:
	- the directional derivitive $$\partial_{\eta}\mathbb{E}[\phi(W,\theta_0,\eta_0)][\eta-\eta_0] = 0$$ for every $$\eta$$ 
	- where $$\partial_{\eta}f(x_0)[x-x_0] := \partial_t f(x_0+t(x-x_0)) $$
...
2. do **Cross-fitting**, where you split your data into a main sample and an auxiliary sample, and:
	...
	- on the main sample, use this trained model to get $$\hat{\eta}$$, and then plug this into the empirical analogue of \eqref{momentcondition} and solve for $$\theta$$, so that
	 $$
	 \mathbb{E}_n[\phi(W, \hat{\theta}, \hat{\eta})] = 0
	 $$
	 implicitly defines $$\hat{\theta}$$
...
- Boosting: the Luo Spindler result assumes the truth is a sparse linear model, and the base learning in the boosting algorithm here are univariate linear regressions, which is not very common in estimating stuff
...
For example, the 'concentrating-out' approach for Neyman orthogonalization in the case of M-estimation with infinite-dimensional nuisance parameters (section 2.23 in the DML paper) requires computing the optimal nuisance parameter $$\eta$$ as a function of the parameter of interest $$\theta$$, and then computing the derivative wrt $$\theta$$ of this function-valued mapping from $$\theta$$ to optimal $$\eta$$.  This approach is then applied to the leading example of interest (partially linear models with nuisance parameter) where the mapping from $$\theta$$ to optimal $$\eta$$ is fairly easy to compute and differentiate, due to the particular functional forms involved.

Overall, it doesn't appear there's a mechanical way to orthogonalize any score function $$\phi$$, and this instead has to be done on a case-by-case basis.  As a result, we'll need to do some manual orthogonalization before we apply do the DML procedure.
