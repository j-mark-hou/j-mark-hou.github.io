...
So, if we want to find $$\hat{f}(x)$$ that approximates $$f(x)$$, a quite reasonable thing to do is to try to pick $$\hat{f}$$ out of some class of functions $$\mathcal{F}$$ to minimize the corresponding empirical average (or sum) of $$L$$ over our data:
...
#### Boosting
Alright, so let's say we're optimiziong some $$L$$.  We'll typically want to do this estimation fairly flexibly, so we'll want $$\mathcal{F}$$ to be fairly complex (as opposed to e.g. considering only linear functions), but complex $$\mathcal{F}$$ makes it harder to do the optimization.  Boosting circumvents this issue by iteratively building up $$\hat{f}$$ via a sum of 'base learners' ( = simple functions).  This approach allows $$\mathcal{F}$$ to be quite complex (as it's a product of a bunch of simple function spaces), but at the cost of making the $$\arg\min$$ approximate:
...
The fact that the splitting procedure 2.2 above appears to require re-computing the loss functions for every dimension of $$x$$ and for every potential cutoff point seems like it would somewhat limit the applicability of tree boosting, espcially for large data sets.  Probably the main innovation of gradient boosting as implemented initially in XGBoost and later in LightGBM/others (and certainly responsible for the popularity of these packages) is how to resolve this issue by using a second-order approximation of the loss function $$L$$.
...
- see [lines 565 onwards](https://github.com/Microsoft/LightGBM/blob/0584065c036414f6e4ae01e72314f2f9cbed7329/src/treelearner/feature_histogram.hpp#L565) for the implementation described above
    ...
    - lines 585 onwards iterates through all the relevant cutpoints
...
- [`CalculateSplittedLeafOutput()`](https://github.com/Microsoft/LightGBM/blob/0584065c036414f6e4ae01e72314f2f9cbed7329/src/treelearner/feature_histogram.hpp#L447) computes the best constant value for the set, denoted by $$\hat{y}$$ above, though there are differences due to some regularization that we didn't discuss above
...
Gradient boosting relies on approximating the true objective $$L$$ with a second-order expansion, and then picking the splits and predictions in order to optimize this approximate loss.  Clearly, this will cause issues if the second order approximation of $$L$$ is bad.  This is exactly the issue with quantile regression.
...
- $$\partial_1L(\tilde{y}, y)$$ are either $$-\alpha$$ or $$1-\alpha$$ depending on $$\tilde{y}<y$$ or $$>$$
- $$\partial_1L(\tilde{y}, y)$$ is 0
- at $$\tilde{y}=y$$, both derivatives are undefined (but this is measure 0 so it doesn't matter)
As a result, you can just... literally plug these into \eqref{gbtyhat} and \eqref{gbtloss} to get the relevant predictions $$\hat{y}$$ and losses $$l$$ (though you would have to add some L2 regularization so that the denominator wasn't uniformly 0).
...
\begin{align}
&\sum_{i\in B}L\left(\sum_{t^\prime<t}\hat{f}_{t^\prime}(x_i) + \tilde{y}, y_i\right)\newline
&= [\mathbb{1}_{\left(\sum_{t^\prime<t}\hat{f}_{t^\prime}(x_i) + \tilde{y}<y\right)}\alpha + \mathbb{1}_{\left(\sum_{t^\prime<t}\hat{f}_{t^\prime}(x_i) + \tilde{y}\geq y\right)}(1-\alpha) ] 
...
so that $$\arg\min_{\tilde{y}}\sum_{i\in B}L\left(\sum_{t^\prime<t}\hat{f}_{t^\prime}(x_i) + \tilde{y}, y_i\right)$$ is just the empirical $$\alpha$$-quantile of $$y_i-\sum_{t^\prime<t}\hat{f}_{t^\prime}(x_i)$$ computed over the set of observations $$B$$.  Thus, **re-computing the optimal $$\hat{y}_B$$ in a given leaf amounts to just finding an empirical quantile.**
...
- during the tree-growing process we're using an approximate loss function that might not be a great approximation for the true one
- this approximate loss can just be seen as a noised up version of the true loss
- as a result, the optimal split might not correspond to the optimal split if we used to true loss
- however, the optimal split is greedy anyways, so it's not going to be globally optimal anyways
...
- so, using the approximate loss is effectively just another way of regularization during the tree growing process
- we only optimize the true loss at the leaves, since those are the only places where the actual values of the loss matters
...
- the [`TrainOneIter()` function](https://github.com/Microsoft/LightGBM/blob/34a643f67527b742ce9084de7616301a82ff890e/src/boosting/gbdt.cpp#L386) implements the logic for training one tree (so, corresponding to a single $$t$$ in our exposition above)
    ...
    - this first calls `tree_learner_->Train()` (line 437)
        - `Train()` function is [implemented e.g. here](https://github.com/Microsoft/LightGBM/blob/master/src/treelearner/serial_tree_learner.cpp#L156), and does the tree-building using the first and second order derivatives
    - and then calls `tree_learner_->RenewTreeOutput()` (line 446)
        - this function does the re-computing of the objective at the leaves
- [This is an implementation of `tree_learner_->RenewTreeOutput()` from above](https://github.com/Microsoft/LightGBM/blob/dc6995742a5284a1e942978e2542fc49adda9ea1/src/treelearner/serial_tree_learner.cpp#L776)
...
- [This is the implementation of `obj->RenewTreeOutput()` for quantile regression](https://github.com/Microsoft/LightGBM/blob/f660b5fe4148fc3f5985b10c1fea969b695cb8de/src/objective/regression_objective.hpp#L512)
    - this just calls the `PercentileFun()` function on each leaf
...
This implementation is consistent we verbally described above: each tree is built normally, and after that finishes, the algorithm goes through each leaf and re-computes the optimal prediction in each leaf, which is just the $$\alpha$$-quantile.
